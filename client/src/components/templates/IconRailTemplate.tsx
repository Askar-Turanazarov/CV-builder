...
import styles from './IconRailTemplate.module.css';

// Unlike SidebarTemplate (a wide column carrying real content — name,
// skills, contacts), the rail here is a narrow icons-only strip: it holds
// nothing but small glyph anchors next to each major section, while content
// occupies almost the full remaining width.
export default function IconRailTemplate({ data }: ResumeTemplateProps) {
  ...
  return (
    <div className={`${styles.page} resume-document ${data.viewMode === 'site' ? styles.site : styles.document}`}>
      <aside className={styles.rail} aria-hidden="true">
        ...
        {data.aiContent?.summary && <span className={styles.railIcon}>📝</span>}
        {data.aiContent && data.aiContent.strengths.length > 0 && <span className={styles.railIcon}>⭐</span>}
        {data.experience.length > 0 && <span className={styles.railIcon}>💼</span>}
        {data.education.length > 0 && <span className={styles.railIcon}>🎓</span>}
        {data.skills.length > 0 && <span className={styles.railIcon}>🛠</span>}
        {data.languages.length > 0 && <span className={styles.railIcon}>🌐</span>}
      </aside>
