...
import styles from './RibbonCornerTemplate.module.css';

// A diagonal ribbon banner carries the job title across the header's top
// corner, and the photo sits in an angular (hexagon-clipped) frame instead
// of the usual circle — an intentionally non-circular, badge-like identity.
export default function RibbonCornerTemplate({ data }: ResumeTemplateProps) {
  ...
  return (
    <div className={`${styles.page} resume-document ${data.viewMode === 'site' ? styles.site : styles.document}`}>
      <header className={styles.header}>
        <div className={styles.ribbon}>{data.personalInfo.jobTitle || ' '}</div>
