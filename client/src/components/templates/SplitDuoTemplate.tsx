...
import styles from './SplitDuoTemplate.module.css';

// Two honest full-height color blocks side by side (not a narrow sidebar):
// left half carries identity + skills on the saturated primary color, right
// half carries the prose (summary/experience/education) on a plain surface.
export default function SplitDuoTemplate({ data }: ResumeTemplateProps) {
    ...
    <div className={`${styles.page} resume-document ${data.viewMode === 'site' ? styles.site : styles.document}`}>
      <div className={styles.left}>
        ...
        {data.skills.length > 0 && (
          <Section title={t('sections.skills')} className={styles.leftSection}>
        ...
        {data.languages.length > 0 && (
          <Section title={t('sections.languages')} className={styles.leftSection}>
            <div className={styles.pillRow}>
