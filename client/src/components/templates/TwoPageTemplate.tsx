...
import styles from './TwoPageTemplate.module.css';

// For long, senior careers that naturally run past one page: comfortable
// (not cramped) reading size, a repeated running header, and generous
// section spacing that lets content flow onto a second printed page cleanly.
export default function TwoPageTemplate({ data }: ResumeTemplateProps) {
  ...
  return (
    <div className={`${styles.page} resume-document`}>
      <header className={styles.runningHeader}>
        <span className={styles.runningName}>{data.personalInfo.fullName || '—'}</span>
        <span className={styles.runningContact}>
          {[data.contacts.email, data.contacts.phone].filter(Boolean).join(' · ')}
        </span>
      </header>
