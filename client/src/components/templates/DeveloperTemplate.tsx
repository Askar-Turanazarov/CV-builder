...
import styles from './DeveloperTemplate.module.css';

// Developer-focused: a dark "editor tab bar" header strip, monospace accents
// on skills/links, GitHub-style contact row.
export default function DeveloperTemplate({ data }: ResumeTemplateProps) {
    ...
    <div className={`${styles.page} resume-document`}>
      <header className={styles.header}>
        <div className={styles.tabBar}>
          ...
          <span className={styles.dot} />
          <span className={styles.dot} />
          <span className={styles.fileName}>{data.personalInfo.fullName || 'resume'}.tsx</span>
        </div>
      ...
      {data.personalInfo.birthDate && (
        <p className={styles.footerNote}>{formatFullDate(data.personalInfo.birthDate, data.uiLanguage)}</p>
      )}
    </div>
  );
}
